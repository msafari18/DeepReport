...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, device, QKVdim):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        scores.to(self.device).masked_fill_(attn_mask, -1e9)
        # [batch_size, n_heads, len_q, len_k]
        attn = nn.Softmax(dim=-1)(scores)
        # [batch_size, n_heads, len_q, QKVdim]
        context = torch.matmul(attn, V).to(self.device)
        return context, attn


class Multi_Head_Attention(nn.Module):
    def __init__(self, device, Q_dim, K_dim, QKVdim, n_heads=8, dropout=0.1):
        super(Multi_Head_Attention, self).__init__()
        self.device = device
        self.W_Q = nn.Linear(Q_dim, QKVdim * n_heads).to(self.device)
        self.W_K = nn.Linear(K_dim, QKVdim * n_heads).to(self.device)
        self.W_V = nn.Linear(K_dim, QKVdim * n_heads).to(self.device)
        self.n_heads = n_heads
        self.QKVdim = QKVdim
        self.embed_dim = Q_dim
        self.dropout = nn.Dropout(p=dropout)
        self.W_O = nn.Linear(self.n_heads * self.QKVdim,
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        context, attn = ScaledDotProductAttention(self.device,
                                                  self.QKVdim)(q_s, k_s, v_s, attn_mask)
        # context: [batch_size, n_heads, len_q, QKVdim] -> [batch_size, len_q, n_heads * QKVdim]
        context = context.transpose(1, 2).contiguous().view(
            batch_size, -1, self.n_heads * self.QKVdim).to(self.device)
        # output: [batch_size, len_q, embed_dim]
        output = self.W_O(context)
        output = self.dropout(output)
        return nn.LayerNorm(self.embed_dim).to(self.device)(output + residual), attn


class PoswiseFeedForwardNet(nn.Module):
    def __init__(self, device, embed_dim, d_ff, dropout):
        ...
        self.conv1 = nn.Conv1d(in_channels=embed_dim,
                               out_channels=d_ff, kernel_size=1).to(self.device)
        self.conv2 = nn.Conv1d(
            in_channels=d_ff, out_channels=embed_dim, kernel_size=1).to(self.device)
        self.dropout = nn.Dropout(p=dropout)
    ...
    def forward(self, inputs):
        ...
        output = self.dropout(output)
        return nn.LayerNorm(self.embed_dim).to(self.device)(output + residual)


class DecoderLayer(nn.Module):
    def __init__(self, device, embed_dim, dropout, attention_method, n_heads):
        super(DecoderLayer, self).__init__()
        self.dec_self_attn = Multi_Head_Attention(device,
                                                  Q_dim=embed_dim, K_dim=embed_dim, QKVdim=64, n_heads=n_heads, dropout=dropout)
        if attention_method == "ByPixel":
            self.dec_enc_attn = Multi_Head_Attention(device,
                                                     Q_dim=embed_dim, K_dim=encoder_dim, QKVdim=64, n_heads=n_heads, dropout=dropout)
            self.pos_ffn = PoswiseFeedForwardNet(device,
                                                 embed_dim=embed_dim, d_ff=encoder_dim, dropout=dropout)
        elif attention_method == "ByChannel":
            self.dec_enc_attn = Multi_Head_Attention(device,
                                                     Q_dim=embed_dim, K_dim=196, QKVdim=64, n_heads=n_heads, dropout=dropout)
            self.pos_ffn = PoswiseFeedForwardNet(device,
                                                 embed_dim=embed_dim, d_ff=encoder_dim, dropout=dropout)  # need to change
...
class Decoder(nn.Module):
    def __init__(self, device, n_layers, vocab_size, embed_dim, dropout, attention_method, n_heads, max_len):
        ...
        self.pos_emb = nn.Embedding.from_pretrained(
            self.get_position_embedding_table(embed_dim), freeze=True)
        self.dropout = nn.Dropout(p=dropout)
        self.layers = nn.ModuleList([DecoderLayer(device,
                                                  embed_dim, dropout, attention_method, n_heads) for _ in range(n_layers)])
        self.projection = nn.Linear(
            embed_dim, vocab_size, bias=False).to(self.device)
    ...
    def get_position_embedding_table(self, embed_dim):
        ...
        embedding_table = np.array(
            [get_posi_angle_vec(pos_i) for pos_i in range(self.max_len)])
        embedding_table[:, 0::2] = np.sin(embedding_table[:, 0::2])  # dim 2i
        embedding_table[:, 1::2] = np.cos(embedding_table[:, 1::2])  # dim 2i+1
        return torch.FloatTensor(embedding_table).to(self.device)
    ...
    def get_attn_subsequent_mask(self, seq):
        ...
        subsequent_mask = torch.from_numpy(
            subsequent_mask).byte().to(self.device)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        dec_outputs = self.tgt_emb(encoded_captions) + self.pos_emb(
            torch.LongTensor([list(range(self.max_len))]*batch_size).to(self.device))
        dec_outputs = self.dropout(dec_outputs)
        dec_self_attn_pad_mask = self.get_attn_pad_mask(
            encoded_captions, encoded_captions)
        dec_self_attn_subsequent_mask = self.get_attn_subsequent_mask(
            encoded_captions)
        dec_self_attn_mask = torch.gt(
            (dec_self_attn_pad_mask + dec_self_attn_subsequent_mask), 0)
        if self.attention_method == "ByPixel":
            dec_enc_attn_mask = (torch.tensor(np.zeros((batch_size, self.max_len, 196))).to(
                self.device) == torch.tensor(np.ones((batch_size, self.max_len, 196))).to(self.device))
        elif self.attention_method == "ByChannel":
            dec_enc_attn_mask = (torch.tensor(np.zeros((batch_size, self.max_len, channel_number))).to(
                self.device) == torch.tensor(np.ones((batch_size, self.max_len, channel_number))).to(self.device))

        dec_self_attns, dec_enc_attns = [], []
        for layer in self.layers:
            # attn: [batch_size, n_heads, len_q, len_k]
            dec_outputs, dec_self_attn, dec_enc_attn = layer(
                dec_outputs, encoder_out, dec_self_attn_mask, dec_enc_attn_mask)
...
class EncoderLayer(nn.Module):
    def __init__(self, device, dropout, attention_method, n_heads):
        super(EncoderLayer, self).__init__()
        """
        In "Attention is all you need" paper, dk = dv = 64, h = 8, N=6
        """
        if attention_method == "ByPixel":
            self.enc_self_attn = Multi_Head_Attention(device,
                                                      Q_dim=encoder_dim, K_dim=encoder_dim, QKVdim=64, n_heads=n_heads, dropout=dropout)
            self.pos_ffn = PoswiseFeedForwardNet(device,
                                                 embed_dim=encoder_dim, d_ff=4096, dropout=dropout)
        elif attention_method == "ByChannel":
            self.enc_self_attn = Multi_Head_Attention(device,
                                                      Q_dim=196, K_dim=196, QKVdim=64, n_heads=n_heads, dropout=dropout)
            self.pos_ffn = PoswiseFeedForwardNet(device,
                                                 embed_dim=196, d_ff=512, dropout=dropout)
...
class Encoder(nn.Module):
    def __init__(self, device, n_layers, dropout, attention_method, n_heads):
        super(Encoder, self).__init__()
        self.device = device
        if attention_method == "ByPixel":
            self.pos_emb = nn.Embedding.from_pretrained(
                self.get_position_embedding_table(), freeze=True)
        # self.dropout = nn.Dropout(p=dropout)
        self.layers = nn.ModuleList(
            [EncoderLayer(device, dropout, attention_method, n_heads) for _ in range(n_layers)])
        self.attention_method = attention_method
    ...
    def get_position_embedding_table(self):
        def cal_angle(position, hid_idx):
            x = position % 14
            y = position // 14
            x_enc = x / np.power(10000, hid_idx / (encoder_dim // 2))
            y_enc = y / np.power(10000, hid_idx / (encoder_dim // 2))
            return np.sin(x_enc), np.sin(y_enc)

        def get_posi_angle_vec(position):
            return [cal_angle(position, hid_idx)[0] for hid_idx in range((encoder_dim // 2))] + [cal_angle(position, hid_idx)[1] for hid_idx in range((encoder_dim // 2))]

        embedding_table = np.array(
            [get_posi_angle_vec(pos_i) for pos_i in range(196)])
        return torch.FloatTensor(embedding_table).to(self.device)
    ...
    def forward(self, encoder_out):
        ...
        if self.attention_method == "ByPixel":
            encoder_out = encoder_out + \
                self.pos_emb(torch.LongTensor(
                    [list(range(positions))]*batch_size).to(self.device))
        # encoder_out = self.dropout(encoder_out)
        # enc_self_attn_mask: [batch_size, 196, 196]
        enc_self_attn_mask = (torch.tensor(np.zeros((batch_size, positions, positions))).to(self.device)
                              == torch.tensor(np.ones((batch_size, positions, positions))).to(self.device))
        enc_self_attns = []
        for layer in self.layers:
...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim, encoder_layers, decoder_layers, dropout=0.1, attention_method="ByPixel", n_heads=8, max_len=200, device=torch.device("cpu")):
        super(Transformer, self).__init__()
        self.device = device
        self.encoder = Encoder(device, encoder_layers, dropout,
                               attention_method, n_heads)
        self.decoder = Decoder(device, decoder_layers, vocab_size,
                               embed_dim, dropout, attention_method, n_heads, max_len=max_len)
    ...
    def forward(self, enc_inputs, encoded_captions, caption_lengths):
        ...
        if self.attention_method == "ByPixel":
            ...
            enc_inputs = enc_inputs.view(
                batch_size, -1, encoder_dim).permute(0, 2, 1)  # (batch_size, 2048, 196)
        ...
        alphas = {"enc_self_attns": enc_self_attns,
                  "dec_self_attns": dec_self_attns, "dec_enc_attns": dec_enc_attns}
